...
class SwitchBlogMenuEvent extends BaseEvent implements EventTarget {

    public function new() {
        super(EVENT_KEY.SWITCH_BLOG_MENU);
    }
    ...
    public function on(listener: (Void -> Dynamic) -> Event -> Void, ?useCaputur = false): Void {
        Browser.window.addEventListener(EnumEvent.to_s(EVENT_KEY.SWITCH_BLOG_MENU), listener.bind(getBindData), useCaputur);
    }

    public function off(listener: Dynamic -> Void): Void {
        Browser.window.removeEventListener(EnumEvent.to_s(EVENT_KEY.SWITCH_BLOG_MENU), listener);
    }
